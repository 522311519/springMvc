...
function addorEditUser() {
	...
	const title = $('#myUser').html();
	if (title.indexOf("修改") !== -1) {
		const id = $('#id').val();
		url = "http://localhost:8080/SpringMvc/user/updateUser?username="
		...
		url = "http://localhost:8080/SpringMvc/user/addUser?username="
			+ username + "&password=" + password + "&phone=" + phone
			+ "&email=" + email;
		$.get(url, function(data) {
			if (data.code === 1) {
 ...
 */
function getUser() {
	const url = "http://localhost:8080/SpringMvc/user/getUser";
	$.get(url, function(data) {
		let html;
		if (data.code === 1) {
			html = template('user_list', data.data);
