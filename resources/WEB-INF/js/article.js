...
function deleteArticle() {
    ...
    const url = "http://localhost:8080/SpringMvc/article/deleteArticle?articleId=" + articleId;
...
function showArticle() {
    const url = "http://localhost:8080/SpringMvc/article/getArticle";
    $.get(url, function (data) {
        if (data.code === 1) {
...
function addorEditArticle() {
    ...
    if (title.indexOf("修改") !== -1) {
        ...
        url = "http://localhost:8080/SpringMvc/article/addArticle";
        obj = {
            'title': article_title,
            'link': article_link
        };
